...
class HybridRetriever:
    ...
    def __init__(
        ...
        if self.enable_agency_mode and repo_root:
            self.logger.info(
                f"Initializing agency mode agents with repo_root: {repo_root}"
            )
            self._initialize_agents(repo_root)
        elif self.enable_agency_mode and not repo_root:
            self.logger.info(
                "Agency mode enabled but repo_root not set yet. Agents will initialize when repository is loaded."
            )
    ...
    def index_for_bm25(self, elements: list[CodeElement]):
        ...
        self.logger.info(
            f"Built full BM25 index with {len(self.full_bm25_corpus)} documents"
        )
    ...
    def build_repo_overview_bm25(self):
        ...
        self.logger.info(
            f"Built repo overview BM25 index with {len(self.repo_overview_bm25_corpus)} repositories"
        )
    ...
    def retrieve(
        ...
        if self.select_repos_by_overview:
            ...
            if len(effective_repos) > 1:
                self.logger.info(
                    f"Multi-repository scenario detected ({len(effective_repos)} repos)"
                )
                ...
                if selected_repos:
                    ...
                    self.logger.info(
                        f"Selected repositories by overview: {repo_filter}"
                    )
                else:
                ...
                self.logger.info(
                    f"Single repository mode ({effective_repos}), skipping LLM repo selection"
                )
        ...
        if should_use_agency and self.iterative_agent:
            ...
            if repo_filter and self.current_loaded_repos != repo_filter:
                self.logger.info(
                    "Reloading specific repository indexes for iterative mode"
                )
        ...
        if repo_filter:
            ...
            if self.current_loaded_repos != repo_filter:
                self.logger.info(
                    "Reloading specific repository indexes for accurate retrieval"
                )
                if self.reload_specific_repositories(repo_filter):
                    self.current_loaded_repos = repo_filter
                else:
                    self.logger.warning(
                        "Failed to reload specific repositories, using filtered search"
                    )
        ...
        semantic_results = self._semantic_search(
            search_text4semantic, top_k=20, repo_filter=repo_filter
        )
        ...
        if pseudocode:
            ...
            self.logger.info(
                f"Pseudocode search found {len(pseudocode_results)} additional results"
            )
        ...
        if enable_file_selection and not should_use_agency:
            self.logger.info(
                "Using file selection for accurate and comprehensive retrieval"
            )
        ...
        if should_use_agency:
            self.logger.info(
                "Using agency mode for accurate and comprehensive retrieval"
            )
            final_results = self._apply_agency_mode(
                query_str, final_results, query_info, repo_filter
            )
    ...
    def _select_relevant_repositories(
        self, query: str | list[str], keywords: list[str] | None, top_k: int = 5
    ) -> list[str]:
        """
        ...
        """
        self.logger.info(
            "Performing repository selection based on overviews (semantic + BM25)"
        )
            ...
            if max_bm25_score > 0:
                for repo_name, score in bm25_results:
                    ...
                    if repo_name in repo_scores:
                        ...
                        repo_scores[repo_name]["total_score"] += (
                            normalized_bm25 * 0.3
                        )  # 30% weight for BM25
                    else:
                        repo_scores[repo_name] = {
        ...
        sorted_repos = sorted(
            qualified_repos, key=lambda x: x[1]["total_score"], reverse=True
        )
    ...
    def _select_relevant_repositories_by_llm(
        self, query: str, top_k: int = 5, scope_repos: list[str] | None = None
    ) -> list[str]:
        """
        ...
        if scope_repos:
            repo_overviews = {
                k: v for k, v in all_overviews.items() if k in scope_repos
            }
            self.logger.info(
            ...
            self.logger.info(
                f"LLM repo selection considering all {len(repo_overviews)} repos"
            )

        if not repo_overviews:
            self.logger.warning(
                "No overviews remain after scoping, searching all scoped repos"
            )
        ...
        try:
            ...
            self.logger.warning(
                "LLM returned no repos, falling back to embedding-based selection"
            )
        except Exception as e:
            self.logger.error(
                f"LLM repo selection failed: {e}, falling back to embedding-based selection"
            )
        ...
        try:
            ...
            if embedding_selected:
                self.logger.info(
                    f"Embedding-based selection returned: {embedding_selected}"
                )
        ...
        if scope_repos:
            self.logger.warning(
                f"All repo selection methods failed, falling back to user's original selection: {scope_repos}"
            )
            return scope_repos

        # Final fallback: return empty list (will search all repos)
        self.logger.warning(
            "All repo selection methods failed and no scope_repos provided, will search all repos"
        )
        return []

    def _enhance_with_file_selection(
        self, query: str, results: list[dict[str, Any]], repo_names: list[str]
    ) -> list[dict[str, Any]]:
        """
    ...
    def _retrieve_elements_from_files(
        self, selected_files: list[dict[str, str]]
    ) -> list[dict[str, Any]]:
        """
        ...
        for file_info in selected_files:
            ...
            for elem in elements_to_search:
                if (
                    ...
                    and file_path in elem.relative_path
                    and elem.type == "file"
    ...
    def _semantic_search(
        self, query: str, top_k: int = 20, repo_filter: list[str] | None = None
        ...
        if (
            self.filtered_vector_store is not None
            and self.filtered_vector_store.get_count() > 0
        ):
    ...
    def _keyword_search(
        self, query: str, top_k: int = 10, repo_filter: list[str] | None = None
    ) -> list[tuple[dict[str, Any], float]]:
        """
        ...
        if filtered_count > 0:
            self.logger.info(
                f"BM25 search filtered out {filtered_count} elements from unexpected repos"
            )
            ...
            if elem_id and elem_id in expanded:
                ...
                for related_id in related_ids:
                    if related_id not in expanded:
                        ...
                        if elem:
                            graph_score = (
                                result["total_score"] * 0.5 * self.graph_weight
                            )
                            expanded[related_id] = {
                                "element": elem.to_dict(),
                                "semantic_score": 0.0,
    ...
    def reload_specific_repositories(self, repo_names: list[str]) -> bool:
        ...
        """
        self.logger.info(
            f"Reloading specific repositories into filtered indexes: {repo_names}"
        )
    ...
    def _calculate_repo_stats(self) -> dict[str, Any] | None:
        """
    ...
    def _apply_agency_mode(
        ...
        try:
            ...
            if self.iterative_agent:
                ...
                final_results, iteration_metadata = (
                    self.iterative_agent.retrieve_with_iteration(
                        query,
                        processed_query,
                        query_info,
                        repo_filter,
            ...
            # Fallback: iterative agent not available
                ...
                self.logger.warning(
                    "Iterative agent not available, returning original results"
                )
                return results
